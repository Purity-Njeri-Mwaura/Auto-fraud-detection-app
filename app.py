...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["📊 SHAP Explanation", "🔍 LIME Explanation", "⚖️ SHAP vs LIME", "📈 Feature Impact", "📋 Claim Summary"]
    )

    # shared helper — defined ONCE, before any tab uses it, fixing the NameError bug
    def clean_feat(feat):
        return feat.replace("_", " ").replace("-", " ").title()

    def clean_lime_rule(rule):
        for op in [" <= ", " >= ", " < ", " > ", " == "]:
            if op in rule:
    ...
    SHAP_CONTEXT = {
        "incident_severity": "Incident severity is the single strongest fraud predictor in auto insurance — Total Loss and Minor Damage claims are disproportionately fraudulent in this dataset.",
        "policy_year": "Older policies filing large claims are a known fraud pattern — fraudsters often wait before making a move.",
        "incident_year": "Temporal shifts in fraud frequency across incident years were learned by the model from historical patterns.",
        "insured_hobbies": "Certain hobbies (chess, skydiving, polo) were statistically linked to fraud rates in the training data — a demographic correlation, not a behavioural cause.",
        ...
        "auto_model": "Certain vehicle models showed local correlation with fraud risk in this dataset.",
        ...
        "collision_type": "Certain collision types (e.g. rear collisions) are more commonly staged than others.",
        "insured_occupation": "Occupation carries a weak but measurable statistical association with fraud in the training data.",
        "policy_number": "Encodes temporal information correlated with policy age and historical fraud patterns.",
    }
    ...
    LIME_CONTEXT = {
        ...
        "incident_year": "The incident year contributed a local temporal signal to this specific prediction.",
        ...
        "auto_model": "The specific vehicle model carried local weight in this prediction.",
        ...
        "collision_type": "The type of collision reported locally affected the fraud score.",
        "insured_occupation": "Occupation carried local weight in this specific prediction.",
    }
    ...
    # cache the heavy computations once so all three tabs (SHAP, LIME, comparison) reuse them
    @st.cache_data(show_spinner=False)
    def compute_shap(input_df_vals, columns):
        import shap as _shap
        _explainer = _shap.TreeExplainer(model)
        _input_df = pd.DataFrame(input_df_vals, columns=columns)
        _sv = _explainer.shap_values(_input_df)
        ...
        if isinstance(ev, (list, np.ndarray)):
            ...
            base_value = float(ev)
        return _sv, base_value

    @st.cache_data(show_spinner=False)
    def compute_lime(input_df_vals, columns):
        np.random.seed(42)
        bg_data = np.tile(input_df_vals[0], (100, 1))
        ...
        _explanation = _lime_explainer.explain_instance(
            input_df_vals[0],
            model.predict_proba,
            num_features=15,
            top_labels=2,
        ...
        available_labels = list(_explanation.available_labels())
        label_to_use = 1 if 1 in available_labels else (available_labels[0] if available_labels else 0)
        return _explanation.as_list(label=label_to_use), _explanation

    sv, base_value = compute_shap(input_df.values, tuple(feature_cols))
    shap_df = pd.DataFrame({"feature": feature_cols, "shap": sv})
    shap_df["abs"] = shap_df["shap"].abs()
    shap_df_top15 = shap_df.sort_values("abs", ascending=False).head(15)

    lime_list, lime_explanation_obj = ([], None)
    lime_error = None
    if LIME_AVAILABLE:
        try:
            lime_list, lime_explanation_obj = compute_lime(input_df.values, tuple(feature_cols))
    ...
    if lime_list:
        ...
        lime_df = pd.DataFrame(columns=["Feature Rule", "Weight"])

    # ══════════════════════════════════════════════════════════════════════
    # TAB 1: SHAP — three charts + rich narrative
    # ══════════════════════════════════════════════════════════════════════
    with tab1:
        st.markdown("""
        **SHAP** (SHapley Additive exPlanations) is grounded in cooperative game theory (Shapley, 1953) and
        adapted to machine learning by Lundberg & Lee (2017). It computes the *exact, mathematically fair*
        contribution of every one of the model's 145 features to this specific prediction, by treating each
        feature as a "player" contributing to the final "payout" (the fraud probability).
        """)

        try:
            tcolor = "#e6edf3" if dark else "#1c2128"
            scolor = "#8b949e"
            bcolor = "#30363d"

            # ── CHART 1: Top-15 SHAP bar chart ──────────────────────────────
            st.markdown('<span class="section-header">📊 Chart 1 — Top 15 Feature Contributions</span>', unsafe_allow_html=True)
            st.markdown('<p style="font-size:.85rem;color:var(--subtext);margin-top:-.5rem">Every bar is one feature\'s exact SHAP value for this claim — how many percentage points of fraud probability it added or removed.</p>', unsafe_allow_html=True)

            fig1, ax1 = plt.subplots(figsize=(9, 6))
            fig1.patch.set_facecolor("none")
            ax1.set_facecolor("none")
            colors1 = ["#f85149" if v > 0 else "#58a6ff" for v in shap_df_top15["shap"]]
            ax1.barh(range(len(shap_df_top15)), shap_df_top15["shap"].values, color=colors1, edgecolor="none", height=0.7)
            ax1.set_yticks(range(len(shap_df_top15)))
            ax1.set_yticklabels([clean_feat(f) for f in shap_df_top15["feature"].values], fontsize=9, color=tcolor)
            ax1.invert_yaxis()
            ax1.set_xlabel("SHAP value (impact on fraud probability)", fontsize=9, color=scolor)
            ax1.axvline(0, color=bcolor, linewidth=1)
            ax1.tick_params(colors=scolor)
            for spine in ax1.spines.values():
                spine.set_edgecolor(bcolor)
            ax1.set_title("Top 15 Features by SHAP Impact — This Claim", fontsize=11, fontweight="bold", color=tcolor, pad=12)
            ...
            ax1.legend(handles=[red_patch, blue_patch], frameon=False, labelcolor=scolor, fontsize=8, loc="lower right")
            plt.tight_layout()
            st.pyplot(fig1)
            plt.close(fig1)
            ...
            <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;
                        padding:.8rem 1.1rem;margin:.4rem 0 1.4rem;font-size:.78rem;color:var(--subtext);line-height:1.6">
                💡 <b style="color:var(--text)">How to read this:</b> Red bars push the prediction toward fraud,
                blue bars push it toward legitimate. Bar length = strength of influence. This chart only shows
                <em>this one claim</em> — a different claim could have a completely different ranking.
            </div>
            """, unsafe_allow_html=True)

            # ── CHART 2: SHAP Waterfall ──────────────────────────────────────
            st.markdown('<span class="section-header">🌊 Chart 2 — Waterfall: How the Verdict Was Built</span>', unsafe_allow_html=True)
            st.markdown('<p style="font-size:.85rem;color:var(--subtext);margin-top:-.5rem">Starting from the average claim in the training data, each feature below pushes the probability up or down until it reaches this claim\'s final score.</p>', unsafe_allow_html=True)

            wf_df = shap_df_top15.head(8).copy()
            wf_df = wf_df.sort_values("abs", ascending=True)
            ...
            ax2.set_yticklabels(labels, fontsize=8.5, color=tcolor)
            ax2.invert_yaxis()
            ax2.axvline(base_value, color=bcolor, linewidth=1, linestyle="--", alpha=0.6)
            ax2.set_xlabel("Cumulative fraud probability (log-odds space)", fontsize=9, color=scolor)
            ax2.tick_params(colors=scolor)
            for spine in ax2.spines.values():
                spine.set_edgecolor(bcolor)
            ax2.set_title("SHAP Waterfall — Step-by-Step Build-Up to This Claim's Verdict",
                          fontsize=10.5, fontweight="bold", color=tcolor, pad=12)
            grey_patch = mpatches.Patch(color="#8b949e", label="Starting point (dataset average)")
            ...
            ax2.legend(handles=[red_patch, blue_patch, grey_patch, gold_patch], frameon=False,
                       labelcolor=scolor, fontsize=7.5, loc="lower right")
            plt.tight_layout()
            st.pyplot(fig2)
            plt.close(fig2)
            ...
            # ── CHART 3: SHAP Global Summary (beeswarm-style approximation) ──
            st.markdown('<span class="section-header">🌍 Chart 3 — Global Feature Importance Across All Claims</span>', unsafe_allow_html=True)
            st.markdown('<p style="font-size:.85rem;color:var(--subtext);margin-top:-.5rem">Beyond this single claim — these are the features that matter most across the model\'s entire 145-feature space, ranked by absolute SHAP magnitude observed in this prediction batch.</p>', unsafe_allow_html=True)

            global_top10 = shap_df.sort_values("abs", ascending=False).head(10).copy()
            fig3, ax3 = plt.subplots(figsize=(9, 5.5))
            fig3.patch.set_facecolor("none")
            ax3.set_facecolor("none")
            norm_vals = global_top10["abs"].values
            max_val = norm_vals.max() if len(norm_vals) else 1
            bar_colors = plt.cm.Reds(0.35 + 0.5 * (norm_vals / max_val)) if dark else plt.cm.Reds(0.3 + 0.55 * (norm_vals / max_val))
            ax3.barh(range(len(global_top10)), norm_vals, color=bar_colors, edgecolor="none", height=0.65)
            ax3.set_yticks(range(len(global_top10)))
            ax3.set_yticklabels([clean_feat(f) for f in global_top10["feature"].values], fontsize=9, color=tcolor)
            ax3.invert_yaxis()
            ax3.set_xlabel("Mean |SHAP value| — overall importance magnitude", fontsize=9, color=scolor)
            ax3.tick_params(colors=scolor)
            for spine in ax3.spines.values():
                spine.set_edgecolor(bcolor)
            ax3.set_title("Top 10 Globally Important Features (by |SHAP| Magnitude)",
                         fontsize=10.5, fontweight="bold", color=tcolor, pad=12)
            plt.tight_layout()
            st.pyplot(fig3)
            plt.close(fig3)
            ...
            <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;
                        padding:.8rem 1.1rem;margin:.4rem 0 1.4rem;font-size:.78rem;color:var(--subtext);line-height:1.6">
                💡 <b style="color:var(--text)">Why this matters:</b> This view answers Research Question 2 of this
                project — <em>which features most strongly predict fraud?</em> Across the full 5-fold cross-validated
                evaluation, incident severity, policy year, incident year, and insured hobbies consistently rank
                as the model's most influential predictors, aligning with known auto insurance fraud indicators.
            </div>
            """, unsafe_allow_html=True)

            # ── Narrative cards ────────────────────────────────────────────
            top_fraud  = shap_df_top15[shap_df_top15["shap"] > 0].head(4)
            top_legit  = shap_df_top15[shap_df_top15["shap"] < 0].head(3)
            ...
            <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;
                ...
                💡 <b style="color:var(--text)">Why SHAP is theoretically rigorous:</b> Unlike simpler feature
                importance methods, SHAP satisfies three mathematical guarantees — local accuracy (the SHAP values
                sum exactly to the difference between this prediction and the average), missingness (features
                with no effect get exactly zero), and consistency (a feature's importance never decreases if the
                model relies on it more). This is why SHAP was selected as the global explainability method for
                this project.
            </div>
            """, unsafe_allow_html=True)
    ...
    # ══════════════════════════════════════════════════════════════════════
    # TAB 2: LIME — three charts + rich narrative
    # ══════════════════════════════════════════════════════════════════════
    with tab2:
        st.markdown("""
        **LIME** (Local Interpretable Model-agnostic Explanations, Ribeiro et al., 2016) treats the model as a
        complete black box. It generates hundreds of slightly perturbed copies of *this exact claim*, observes
        how the model's prediction shifts, and fits a simple local linear model to explain what mattered —
        purely for this one case, independent of how SHAP explains it.
        """)
        ...
        if not LIME_AVAILABLE:
            ...
            st.error(f"LIME error: {lime_error}")
        elif lime_df.empty:
            st.warning("LIME returned no weights for this claim.")
        else:
            # ── CHART 1: LIME bar chart ──────────────────────────────────────
            st.markdown('<span class="section-header">🔍 Chart 1 — Local Feature Weights</span>', unsafe_allow_html=True)
            st.markdown('<p style="font-size:.85rem;color:var(--subtext);margin-top:-.5rem">Each bar is a feature condition that was tested by perturbing this claim hundreds of times — the weight shows how much that exact condition moved the prediction.</p>', unsafe_allow_html=True)

            fig1, ax1 = plt.subplots(figsize=(9, 6))
            fig1.patch.set_facecolor("none")
            ax1.set_facecolor("none")
            colors1 = ["#f97316" if w > 0 else "#3b82f6" for w in lime_df["Weight"]]
            ax1.barh(range(len(lime_df)), lime_df["Weight"].values, color=colors1, edgecolor="none", height=0.7)
            ax1.set_yticks(range(len(lime_df)))
            ax1.set_yticklabels(lime_df["Feature Rule"].values, fontsize=8, color=tcolor)
            ax1.invert_yaxis()
            ax1.set_xlabel("LIME weight (local impact on fraud probability)", fontsize=9, color=scolor)
            ax1.axvline(0, color=bcolor, linewidth=1)
            ax1.tick_params(colors=scolor)
            for spine in ax1.spines.values():
                spine.set_edgecolor(bcolor)
            ax1.set_title("LIME — Local Feature Explanation (Fraud Class)", fontsize=11, fontweight="bold", color=tcolor, pad=12)
            orange_patch = mpatches.Patch(color="#f97316", label="Pushes toward Fraud")
            blue_patch2  = mpatches.Patch(color="#3b82f6", label="Pushes toward Legitimate")
            ax1.legend(handles=[orange_patch, blue_patch2], frameon=False, labelcolor=scolor, fontsize=8)
            plt.tight_layout()
            st.pyplot(fig1)
            plt.close(fig1)
            ...
            # ── CHART 2: LIME confidence / weight magnitude ranking ─────────
            st.markdown('<span class="section-header">📐 Chart 2 — Condition Strength Ranking</span>', unsafe_allow_html=True)
            st.markdown('<p style="font-size:.85rem;color:var(--subtext);margin-top:-.5rem">The same conditions, ranked purely by strength (absolute weight) regardless of direction — showing which conditions LIME found most decisive overall.</p>', unsafe_allow_html=True)

            lime_strength = lime_df.copy()
            lime_strength["abs_weight"] = lime_strength["Weight"].abs()
            lime_strength = lime_strength.sort_values("abs_weight", ascending=False).head(10)

            fig2, ax2 = plt.subplots(figsize=(9, 5.5))
            fig2.patch.set_facecolor("none")
            ax2.set_facecolor("none")
            norm_vals = lime_strength["abs_weight"].values
            max_val = norm_vals.max() if len(norm_vals) else 1
            bar_colors = plt.cm.Oranges(0.35 + 0.55 * (norm_vals / max_val))
            ax2.barh(range(len(lime_strength)), norm_vals, color=bar_colors, edgecolor="none", height=0.65)
            ax2.set_yticks(range(len(lime_strength)))
            ax2.set_yticklabels(lime_strength["Feature Rule"].values, fontsize=8.5, color=tcolor)
            ax2.invert_yaxis()
            ax2.set_xlabel("Absolute LIME weight — decisiveness for this claim", fontsize=9, color=scolor)
            ax2.tick_params(colors=scolor)
            for spine in ax2.spines.values():
                spine.set_edgecolor(bcolor)
            ax2.set_title("Top 10 Most Decisive Conditions for This Claim", fontsize=10.5, fontweight="bold", color=tcolor, pad=12)
            plt.tight_layout()
            st.pyplot(fig2)
            plt.close(fig2)

            # ── CHART 3: LIME local model fidelity (R²) ──────────────────────
            st.markdown('<span class="section-header">🎯 Chart 3 — Local Model Fidelity</span>', unsafe_allow_html=True)
            st.markdown('<p style="font-size:.85rem;color:var(--subtext);margin-top:-.5rem">LIME\'s explanation is itself a simplified model — this score shows how well that simple local model actually approximates the real LightGBM prediction near this claim.</p>', unsafe_allow_html=True)

            try:
                fidelity_score = lime_explanation_obj.score
            except Exception:
                fidelity_score = None
            ...
            if fidelity_score is not None:
                ...
                fig3.patch.set_facecolor("none")
                ax3.set_facecolor("none")
                ax3.barh([0], [1], color="#30363d" if dark else "#d0d7de", height=0.4)
                fid_color = "#3fb950" if fidelity_score > 0.7 else ("#d29922" if fidelity_score > 0.4 else "#f85149")
                ax3.barh([0], [max(min(fidelity_score, 1), 0)], color=fid_color, height=0.4)
                ax3.set_xlim(0, 1)
                ax3.set_yticks([])
                ax3.set_xticks([0, 0.25, 0.5, 0.75, 1.0])
                ax3.tick_params(colors=scolor, labelsize=8)
                for spine in ax3.spines.values():
                    spine.set_edgecolor(bcolor)
                ax3.set_title(f"Local R² Fidelity Score: {fidelity_score:.3f}", fontsize=10, fontweight="bold", color=tcolor, pad=8)
                plt.tight_layout()
                st.pyplot(fig3)
                plt.close(fig3)

                fidelity_word = "high" if fidelity_score > 0.7 else ("moderate" if fidelity_score > 0.4 else "low")
                st.markdown(f"""
                <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;
                            padding:.8rem 1.1rem;margin:.4rem 0 1.4rem;font-size:.78rem;color:var(--subtext);line-height:1.6">
                    💡 <b style="color:var(--text)">What this means:</b> A fidelity score of <b style="color:var(--text)">{fidelity_score:.3f}</b>
                    indicates <b style="color:var(--text)">{fidelity_word} fidelity</b> — how closely LIME's simple linear
                    approximation matches LightGBM's actual (much more complex) decision boundary near this claim.
                    Scores closer to 1.0 mean the explanation can be trusted more strongly; lower scores suggest the
                    model's true behaviour near this claim is harder to approximate with a straight line.
                </div>
                """, unsafe_allow_html=True)
            else:
                st.info("Fidelity score not available for this explanation instance.")
            ...
            <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;
                        padding:.9rem 1.2rem;margin-top:1rem;font-size:.8rem;color:var(--subtext);line-height:1.6">
                💡 <b style="color:var(--text)">Why LIME complements SHAP:</b> SHAP guarantees mathematical
                consistency but requires access to the model's internal tree structure. LIME makes no such
                assumption — it would work even if LightGBM were swapped for a completely different black-box
                model. Using both together, as this project does, provides triangulated evidence: when SHAP and
                LIME agree, confidence in the explanation is much higher than relying on either method alone.
            </div>
            """, unsafe_allow_html=True)

    # ══════════════════════════════════════════════════════════════════════
    # TAB 3: SHAP vs LIME — dedicated standalone comparison tab
    # ══════════════════════════════════════════════════════════════════════
    with tab3:
        st.markdown("""
        This tab directly answers **Research Question 3** of this project: *how consistent and interpretable
        are SHAP and LIME explanations for individual auto insurance claim predictions?* Two independent
        explanation methods — one global and game-theoretic, one local and perturbation-based — are compared
        side by side on the exact same claim.
        """)
        ...
        if lime_df.empty:
            st.warning("LIME results are unavailable, so a comparison cannot be generated for this claim.")
        else:
            try:
                shap_top = shap_df.sort_values("abs", ascending=False).head(8).copy()
                ...
                lime_top["clean_name"] = lime_top["Feature Rule"].apply(clean_lime_rule)
                ...
                shap_lookup = dict(zip(shap_top["clean_name"], shap_top["shap"]))
                lime_lookup = dict(zip(lime_top["clean_name"], lime_top["value"]))

                shap_norm = max((abs(v) for v in shap_lookup.values()), default=1) or 1
                lime_norm = max((abs(v) for v in lime_lookup.values()), default=1) or 1
                ...
                st.markdown('<span class="section-header">⚖️ Chart 1 — Side-by-Side Feature Comparison</span>', unsafe_allow_html=True)
                st.markdown('<p style="font-size:.85rem;color:var(--subtext);margin-top:-.5rem">Each method\'s top features, scaled to its own strongest value so the shapes — not the raw units — can be compared fairly.</p>', unsafe_allow_html=True)
                ...
                bar_h = 0.35
                ax1.barh(y_idx + bar_h/2, shap_plot_vals, height=bar_h, color="#58a6ff", label="SHAP (normalised)", edgecolor="none")
                ax1.barh(y_idx - bar_h/2, lime_plot_vals, height=bar_h, color="#f97316", label="LIME (normalised)", edgecolor="none")
                ax1.set_yticks(y_idx)
                ax1.set_yticklabels(all_feats, fontsize=8.5, color=tcolor)
                ax1.invert_yaxis()
                ax1.axvline(0, color=bcolor, linewidth=1)
                ax1.set_xlabel("Relative influence (each method scaled to its own strongest feature)", fontsize=8.5, color=scolor)
                ax1.tick_params(colors=scolor)
                for spine in ax1.spines.values():
                    spine.set_edgecolor(bcolor)
                ax1.set_title("SHAP vs LIME — Top Features Compared on the Same Claim", fontsize=10.5, fontweight="bold", color=tcolor, pad=12)
                ax1.legend(frameon=False, labelcolor=scolor, fontsize=8.5, loc="lower right")
                plt.tight_layout()
                st.pyplot(fig1)
                plt.close(fig1)

                overlap = set(shap_top["clean_name"]).intersection(set(lime_top["clean_name"]))
                union_feats = set(shap_top["clean_name"]).union(lime_top["clean_name"])
                agreement_pct = (len(overlap) / max(len(union_feats), 1)) * 100

                # ── CHART 2: Agreement gauge ─────────────────────────────────
                st.markdown('<span class="section-header">🤝 Chart 2 — Agreement Score</span>', unsafe_allow_html=True)
                st.markdown('<p style="font-size:.85rem;color:var(--subtext);margin-top:-.5rem">What fraction of each method\'s top 8 features overlap with the other\'s top 8 — a simple, transparent consistency metric.</p>', unsafe_allow_html=True)

                fig2, ax2 = plt.subplots(figsize=(7, 2.2))
                fig2.patch.set_facecolor("none")
                ax2.set_facecolor("none")
                ax2.barh([0], [100], color="#30363d" if dark else "#d0d7de", height=0.45)
                agree_color = "#3fb950" if agreement_pct >= 60 else ("#d29922" if agreement_pct >= 35 else "#f85149")
                ax2.barh([0], [agreement_pct], color=agree_color, height=0.45)
                ax2.set_xlim(0, 100)
                ax2.set_yticks([])
                ax2.set_xticks([0, 25, 50, 75, 100])
                ax2.tick_params(colors=scolor, labelsize=8)
                for spine in ax2.spines.values():
                    spine.set_edgecolor(bcolor)
                ax2.set_title(f"Agreement Score: {agreement_pct:.0f}%", fontsize=11, fontweight="bold", color=tcolor, pad=8)
                plt.tight_layout()
                st.pyplot(fig2)
                plt.close(fig2)
                ...
                st.markdown(f"""
                <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;
                            padding:1rem 1.4rem;margin:.8rem 0 1.4rem;font-size:.88rem;color:var(--text);line-height:1.6">
                    🤝 <b>{agreement_pct:.0f}% of the top features overlap</b> between SHAP and LIME for this claim:
                    {", ".join(sorted(overlap)) if overlap else "no shared top features were found"}.
                    Higher overlap means both explanation methods — one global and game-theoretic, one local and
                    perturbation-based — independently arrived at the same reasoning, which is strong evidence the
                    prediction is trustworthy rather than an artefact of one particular method.
                </div>
                """, unsafe_allow_html=True)

                # ── CHART 3: Rank correlation scatter ─────────────────────────
                st.markdown('<span class="section-header">📈 Chart 3 — Rank Correlation</span>', unsafe_allow_html=True)
                st.markdown('<p style="font-size:.85rem;color:var(--subtext);margin-top:-.5rem">For features that appear in both top-8 lists, this shows whether the two methods also rank them in a similar order of importance.</p>', unsafe_allow_html=True)

                shared = sorted(overlap)
                if len(shared) >= 2:
                    shap_ranks = {f: i for i, f in enumerate(shap_top["clean_name"].tolist())}
                    lime_ranks = {f: i for i, f in enumerate(lime_top["clean_name"].tolist())}
                    xs = [shap_ranks[f] for f in shared]
                    ys = [lime_ranks[f] for f in shared]

                    fig3, ax3 = plt.subplots(figsize=(6, 6))
                    fig3.patch.set_facecolor("none")
                    ax3.set_facecolor("none")
                    ax3.scatter(xs, ys, s=140, color="#a78bfa", edgecolor=tcolor, linewidth=0.8, zorder=3)
                    for f, x, y in zip(shared, xs, ys):
                        ax3.annotate(f, (x, y), textcoords="offset points", xytext=(6, 6), fontsize=7.5, color=tcolor)
                    max_r = max(max(xs, default=0), max(ys, default=0)) + 1
                    ax3.plot([0, max_r], [0, max_r], linestyle="--", color=bcolor, linewidth=1, label="Perfect agreement line")
                    ax3.set_xlabel("SHAP rank (0 = most important)", fontsize=9, color=scolor)
                    ax3.set_ylabel("LIME rank (0 = most important)", fontsize=9, color=scolor)
                    ax3.set_title("Shared Features — Rank Agreement", fontsize=10.5, fontweight="bold", color=tcolor, pad=12)
                    ax3.tick_params(colors=scolor)
                    for spine in ax3.spines.values():
                        spine.set_edgecolor(bcolor)
                    ax3.legend(frameon=False, labelcolor=scolor, fontsize=8)
                    ax3.invert_yaxis()
                    ax3.invert_xaxis()
                    plt.tight_layout()
                    st.pyplot(fig3)
                    ...
                    <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;
                                padding:.8rem 1.1rem;margin:.4rem 0 1.4rem;font-size:.78rem;color:var(--subtext);line-height:1.6">
                        💡 <b style="color:var(--text)">How to read this:</b> Points sitting close to the dashed
                        diagonal line mean both methods rank that feature similarly in importance. Points far from
                        the line indicate disagreement — for example a feature SHAP considers the single most
                        important driver but LIME ranks lower, or vice versa.
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.info("Fewer than two shared top features — not enough overlap to plot a meaningful rank comparison for this claim.")

                # ── Closing methodological note ────────────────────────────
                st.markdown(f"""
                <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;
                            padding:1.2rem 1.5rem;margin-top:1rem;font-size:.88rem;color:var(--text);line-height:1.7">
                    📚 <b>Why this comparison matters for this project:</b> A system that only used one
                    explainability method would be vulnerable to that method's individual blind spots. SHAP
                    is mathematically rigorous but explains the model's global behaviour; LIME is model-agnostic
                    but only locally faithful and can vary between runs. By running both methods on every claim
                    and reporting their agreement explicitly, FraudShield provides investigators with two
                    independent lines of evidence rather than a single black-box score — directly addressing
                    the project's core objective of building a <b>transparent</b>, not just accurate, fraud
                    detection system.
                </div>
                """, unsafe_allow_html=True)
