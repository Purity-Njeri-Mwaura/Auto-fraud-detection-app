...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📊 SHAP Explanation", "🔍 LIME Explanation", "⚖️ SHAP vs LIME", "📋 Claim Summary"]
    )
    ...
    with tab1:
        ...
        **SHAP** (SHapley Additive exPlanations) is grounded in cooperative game theory (Shapley, 1953) and
        ...
        try:
            ...
            # ── DYNAMIC NARRATIVE — auto-generated from this claim's actual SHAP values ──
            top_driver = shap_df_top15.iloc[0]
            second_driver = shap_df_top15.iloc[1] if len(shap_df_top15) > 1 else None
            top_driver_name = clean_feat(top_driver["feature"])
            top_driver_dir = "increased" if top_driver["shap"] > 0 else "decreased"

            if second_driver is not None:
                second_name = clean_feat(second_driver["feature"])
                second_dir = "also increased" if second_driver["shap"] > 0 else "partially offset this by decreasing"
                narrative = (
                    f"For this specific claim, <b>{top_driver_name}</b> was the single strongest factor, "
                    f"which {top_driver_dir} the fraud probability by {abs(top_driver['shap']):.3f}. "
                    f"<b>{second_name}</b> {second_dir} it by a further {abs(second_driver['shap']):.3f}."
                )
            else:
                narrative = (
                    f"For this specific claim, <b>{top_driver_name}</b> was the dominant factor, "
                    f"which {top_driver_dir} the fraud probability by {abs(top_driver['shap']):.3f}."
                )

            st.markdown(f"""
            <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;
                        padding:.9rem 1.2rem;margin:.6rem 0 1.4rem;font-size:.85rem;color:var(--text);line-height:1.6">
                🧭 <b>In plain terms:</b> {narrative} The final probability after all 145 features are accounted
                for is <b>{prob*100:.1f}%</b>.
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        **LIME** (Local Interpretable Model-agnostic Explanations, Ribeiro et al., 2016) treats the model as a
        ...
        if not LIME_AVAILABLE:
            ...
            # ── DYNAMIC NARRATIVE — auto-generated from this claim's actual LIME weights ──
            if not lime_df.empty:
                top_lime = lime_df.iloc[0]
                top_lime_name = clean_feat(clean_lime_rule(top_lime["Feature Rule"]))
                top_lime_dir = "toward fraud" if top_lime["Weight"] > 0 else "toward legitimate"
                st.markdown(f"""
                <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;
                            padding:.9rem 1.2rem;margin:.6rem 0 1.4rem;font-size:.85rem;color:var(--text);line-height:1.6">
                    🧭 <b>In plain terms:</b> Locally, LIME found that <b>{top_lime_name}</b> was the strongest
                    individual signal for this claim, pushing the prediction <b>{top_lime_dir}</b> with a weight
                    of {top_lime['Weight']:.3f}. This is LIME's own independent read on the claim — it does not
                    look at the model's internals at all, only at how the prediction changes when this claim is
                    slightly perturbed.
                </div>
                """, unsafe_allow_html=True)


            st.markdown('<span class="section-header">🎯 Chart 2 — Local Model Fidelity</span>', unsafe_allow_html=True)
    ...
    with tab3:
        ...
        if lime_df.empty:
            ...
            try:
                ...
                # ── DYNAMIC NOTE on shared features, replacing the scatter chart ──
                shared = sorted(overlap)
                if shared:
                    st.markdown(f"""
                    <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;
                                padding:.9rem 1.2rem;margin:.4rem 0 1.4rem;font-size:.85rem;color:var(--text);line-height:1.6">
                        🧭 <b>In plain terms:</b> For this claim, both SHAP and LIME independently pointed to
                        <b>{", ".join(shared)}</b> as influential. When two structurally different explanation
                        methods agree on the same features without being told to, that is strong evidence the
                        signal is real rather than a quirk of one method.
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.info("No shared top features were found between SHAP and LIME for this specific claim.")
    ...
    # ── TAB 4: Claim Summary ──────────────────────────────────────────────────
    with tab4:
        # ── Dynamic summary paragraph combining SHAP + LIME ──────────────
        top_shap_feat = clean_feat(shap_df_top15.iloc[0]["feature"])
        top_shap_val  = shap_df_top15.iloc[0]["shap"]
        verdict_word  = "fraudulent" if pred == 1 else "legitimate"
        shap_dir      = "raised" if top_shap_val > 0 else "lowered"

        if not lime_df.empty:
            top_lime_feat = clean_feat(clean_lime_rule(lime_df.iloc[0]["Feature Rule"]))
            lime_clause = f" Independently, LIME's local analysis agreed that <b>{top_lime_feat}</b> was the most decisive factor for this individual claim."
        else:
            lime_clause = ""

        st.markdown(f"""
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;
                    padding:1.2rem 1.5rem;margin-bottom:1.2rem;font-size:.92rem;color:var(--text);line-height:1.7">
            📋 <b>Summary:</b> This claim was assessed as <b>{verdict_word}</b> with a fraud probability of
            <b>{prob*100:.1f}%</b>. The single strongest global driver was <b>{top_shap_feat}</b>, which
            {shap_dir} the fraud probability by {abs(top_shap_val):.3f} according to SHAP.{lime_clause}
        </div>
        """, unsafe_allow_html=True)
