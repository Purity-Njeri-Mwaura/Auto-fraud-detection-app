...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab2:
        ...
        if not LIME_AVAILABLE:
            ...
            try:
                # Use zeros as background — safe, deterministic, no random noise issues
                bg_data = np.zeros((100, len(feature_cols)))
                ...
                # get available labels and use the first one
                available_labels = explanation.available_labels()
                label_to_use = available_labels[0] if available_labels else 1
                ...
                import traceback
                st.error(f"LIME computation error: {type(e).__name__}: {e}")
